...
// DEMO_MAP_ID is Google's official test map ID required by AdvancedMarkerElement
...
/* ── Bootstrap loader — uses loading=async (no callback pattern) ── */
function loadMapsBootstrap(onReady: () => void) {
  if (window.__mapsApiLoaded && window.google?.maps?.importLibrary) {
    onReady();
    return;
  }
  if (!window.__mapsApiCallbacks) window.__mapsApiCallbacks = [];
...
/* ── Parse old Geocoder address_components (long_name) ── */
...
/* ── Parse new Places API addressComponents (longText) ── */
...
export default function GoogleMapPicker({
  ...
  /* ── Reverse geocode using Promise-based Geocoder ── */
    ...
    async (lat: number, lng: number) => {
      ...
      try {
        ...
        if (results?.[0]) {
          ...
          onSelect({ lat, lng, ...parseGeocoderComponents(results[0].address_components ?? []) });
        } else {
          onSelect({ lat, lng });
        }
      } catch {
  ...
  /* ── Initialize using importLibrary — fixes loading=async + all deprecation warnings ── */
  const initMap = useCallback(async () => {
    if (initializedRef.current || !mapDivRef.current || !window.google?.maps?.importLibrary) return;
    ...
    try {
      ...
      /* dragend: use marker.position (AdvancedMarkerElement has no event.latLng) */
      ...
      if (acContainerRef.current) {
        ...
        placeAC.addEventListener("gmp-placeselect", async (e: any) => {
          try {
            const { place } = e;
            ...
            onSelect({ lat, lng, ...parsePlaceComponents(place.addressComponents ?? []) });
          } catch { /* ignore */ }
        });
      }
    ...
    <div className="space-y-2">
      ...
      <div className="flex gap-2 items-center">
        <div className="flex-1 min-w-0">
          {/* Placeholder shown while Maps API loads */}
          {!isLoaded && (
          ...
          {/* PlaceAutocompleteElement web component is appended here */}
          <div ref={acContainerRef} className={!isLoaded ? "hidden" : "w-full"} />
        </div>
        <button
          type="button"
