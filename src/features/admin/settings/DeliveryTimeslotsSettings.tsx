...
import { 
    ...
    AlertCircle, Check, X, ShieldAlert, ChevronDown, Info
} from "lucide-react";
...
export const DeliveryTimeslotsSettings: React.FC = () => {
    ...
    const handleCreateSlot = async () => {
        ...
        try {
            ...
            setSlotForm({ name: "", start_time: "08:00", end_time: "09:00", cutoff_time: "07:30", sort_order: slots.length + 1 });
            loadData();
        } catch (err) {
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div className="flex bg-stone-100 p-1 rounded-2xl w-fit">
                <button
                    ...
                    className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === "slots" ? "bg-white text-black shadow-sm" : "text-stone-500 hover:text-black"}`}
                >
                    Global Timeslots
                </button>
                <button
                    onClick={() => setActiveTab("overrides")}
                    className={`px-6 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === "overrides" ? "bg-white text-black shadow-sm" : "text-stone-500 hover:text-black"}`}
                >
                    Date Overrides
                </button>
            </div>
            ...
            {activeTab === "slots" ? (
                <div className="bg-white rounded-3xl border border-stone-100 shadow-sm overflow-hidden">
                    <div className="p-6 border-b border-stone-50 bg-stone-50/50 flex justify-between items-center">
                        ...
                        <button
                            ...
                            className="bg-black text-white px-4 py-2 rounded-xl text-xs font-bold flex items-center gap-2 hover:bg-stone-800 transition-all"
                        >
                    ...
                    <div className="p-6">
                        {isAddingSlot && (
                            <div className="mb-8 p-6 bg-stone-50 rounded-2xl border border-stone-200 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end animate-in zoom-in-95 duration-200">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-stone-50">
                                    {slots.length === 0 ? (
                                        <tr>
                                            <td colSpan={5} className="py-12 text-center text-stone-400 text-sm">No timeslots configured yet.</td>
                                        </tr>
                                    ) : (
                                        slots.map((slot) => (
                                            <tr key={slot.id} className="hover:bg-stone-50/50 transition-colors group">
                ...
                <div className="bg-white rounded-3xl border border-stone-100 shadow-sm overflow-hidden">
                    ...
                    <div className="p-6">
                        {isAddingOverride && (
                            <div className="mb-8 p-6 bg-amber-50 rounded-2xl border border-amber-200 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end animate-in zoom-in-95 duration-200">
                                <div className="space-y-1.5">
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                ...
                                <tbody className="divide-y divide-stone-50">
                                    {overrides.length === 0 ? (
                                        <tr>
                                            <td colSpan={4} className="py-12 text-center text-stone-400 text-sm">No date-specific overrides active.</td>
                                        </tr>
                                    ) : (
                                        overrides.map((ov) => {
